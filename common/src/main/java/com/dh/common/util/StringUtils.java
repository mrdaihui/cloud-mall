...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    /**
     * 用guava判断传入的参数是否为null或者空字符串，为null或者空字符串返回true
     *
     * @param args
     * @return
     */
    public static boolean isNullOrEmptyStr(String... args) {
        for (String arg : args) {
    ...
    public static boolean isNullOrEmptyStr(Object... args) {
        for (Object arg : args) {
            if (arg == null || Strings.isNullOrEmpty(arg.toString())) {
                return true;
            }
        }
        return false;
    ...
    /**
     * 应用上报（应用领域类型为1-9的值改为两位显示，如‘1’->‘01’）
     *
     * @param type
     * @return
     */
    public static String getRetype(String type) {
        if (Integer.parseInt(type) >= 0 && Integer.parseInt(type) <= 9) {
            return "0"+type;
        }
        return type;
    }
